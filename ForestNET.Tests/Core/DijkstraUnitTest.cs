namespace ForestNET.Tests.Core
{
    public class DijkstraUnitTest
    {
        ...
        public void TestDijkstra()
        {
            ...
            ForestNET.Lib.Dijkstra<int> o_graphInteger = new();
            ...
            for (int i = 0; i < o_graphInteger.Mapping.Count; i++)
            {
                Assert.That(
                    ...
                    Is.True,
                    "shortest path " + ForestNET.Lib.Helper.PrintArrayList<int>(o_graphInteger.ShortestPathTo(i + 1) ?? []) + " from '" + i_startNode + "' to '" + (i + 1) + "' is not '" + ForestNET.Lib.Helper.PrintArrayList<int>(a_shortestPathsToIntegerResult[i]) + "'"
                );
            }
            ...
            ForestNET.Lib.Dijkstra<string> o_graph = new();
            ...
            for (int i = 0; i < a_locations.Count; i++)
            {
                Assert.That(
                    ...
                    Is.True,
                    "shortest path " + ForestNET.Lib.Helper.PrintArrayList<string>(o_graph.ShortestPathTo(a_locations[i]) ?? []) + " from '" + s_startNode + "' to '" + a_locations[i] + "' is not '" + ForestNET.Lib.Helper.PrintArrayList<string>(a_shortestPathsToResult[i]) + "'"
                );
            }
        }
    }
