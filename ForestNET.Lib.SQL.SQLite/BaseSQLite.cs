...
namespace ForestNET.Lib.SQL.SQLite
{
    /// <summary>
    /// BaseSQLite class for sql connection to a database and fetching queries to it.
    /// requires ADO.NET(Microsoft.Data.Sqlite).
    /// </summary>
    public class BaseSQLite : Base
    {

        /* Fields */

        private SQLiteConnection? o_currentConnection = null;
        private SQLiteCommand? o_currentStatement = null;
        private SQLiteTransaction? o_currentTransaction = null;
        private SQLiteDataReader? o_currentResult = null;
        ...
        /// <summary>
        /// BaseSQLite constructor, initiating database server connection
        /// </summary>
        /// <param name="p_s_host">file-location of sqlite3 database</param>
        ...
        public BaseSQLite(string p_s_host)
        {
            ...
            try
            {
                ForestNET.Lib.Global.ILogConfig("try database connection with: '" + p_s_host + "'");

                this.o_currentConnection = new("Data Source=" + p_s_host);
                this.o_currentConnection.Open();
                this.o_currentTransaction = this.o_currentConnection.BeginTransaction();

                this.s_tempConnectionString = p_s_host;


                ForestNET.Lib.Global.ILogConfig("database connection to '" + BaseGateway.SQLITE + "' established");
            }
        ...
        public override bool TestConnection()
        {
            try
            {
                ...
                (this.o_currentStatement ?? throw new NullReferenceException("could not create and use sql statement")).CommandText = "SELECT 1"; /* or 'SELECT name FROM sqlite_schema' */
                ...
                using SQLiteDataReader o_tempResult = this.o_currentStatement.ExecuteReader();
        ...
        public override void CloseConnection()
        {
            ...
            {
                try { this.o_currentConnection.Close(); this.o_currentConnection.Dispose(); SQLiteConnection.ClearAllPools(); } catch (Exception) { /* ignored */ }
        ...
        public override List<Dictionary<string, Object?>> FetchQuery(IQuery? p_o_sqlQuery, bool p_b_autoCommit)
        {
            ...
            if (p_o_sqlQuery?.BaseGateway != BaseGateway.SQLITE)
            {
                throw new AccessViolationException("Query has invalid BaseGateway setting: '" + p_o_sqlQuery?.BaseGateway + "(query)' != '" + BaseGateway.SQLITE + "(base)'");
            }
            ...
            try
            {
                ...
                for (int i = 0; i < a_queries.Length; i++)
                {
                    if (!ForestNET.Lib.Helper.IsStringEmpty(a_queries[i]))
                    {
                        ...
                        string s_queryFoo = Query<Column>.ConvertToPreparedStatementQuery(BaseGateway.SQLITE, a_queries[i].ToString(), a_values, false);
                        ...
                        if ((p_b_autoCommit) && (i == (a_queries.Length - 1)) && (a_queries[i].ToString().Equals("VACUUM")))
                        {
                            /* commit our transaction */
                            this.o_currentTransaction?.Commit();

                            ForestNET.Lib.Global.ILogConfig("reopen database connection for '" + BaseGateway.SQLITE + "', because 'VACUUM' has been used");

                            /* reopen our connection */
                            this.CloseConnection();
                            this.o_currentConnection = new("Data Source=" + this.s_tempConnectionString);
                            this.o_currentConnection.Open();
                        ...
                        foreach (KeyValuePair<string, Object> o_valuePair in a_values)
                        {
                            ...
                            else if (s_type.Equals("localtime"))
                            {
                                ...
                                if (o_foo is TimeSpan o_timeSpan) /* handle object as TimeSpan */
                                {
                                    this.o_currentStatement.Parameters.AddWithValue("@" + i_indexOfValue.ToString("D5"), o_timeSpan.ToString("hh\\:mm\\:ss"));
                                }
                                else /* handle object as string */
                                {
                                    this.o_currentStatement.Parameters.AddWithValue("@" + i_indexOfValue.ToString("D5"), o_foo.ToString());
                        ...
                        else
                        {
                            ...
                            catch (SQLiteException)
                            {
                                ...
                                {
                                    if (ForestNET.Lib.Global.IsILevel((byte)ForestNET.Lib.Log.Level.MASS)) ForestNET.Lib.Global.ILogMass("execute prepared alter statement for " + BaseGateway.SQLITE);
                                    this.o_currentResult = this.o_currentStatement.ExecuteReader();
                                    if (ForestNET.Lib.Global.IsILevel((byte)ForestNET.Lib.Log.Level.MASS)) ForestNET.Lib.Global.ILogMass("prepared alter statement executed for " + BaseGateway.SQLITE);
                                }
                                else
                                {
                                    throw;
            ...
            else
            {
                ...
                string s_lastInsertIdQuery = "SELECT last_insert_rowid()";
        ...
        /// <exception cref="SqliteException">exception accessing column type, column name or just column value of current result set record</exception>
        private Dictionary<string, Object?> FetchRow()
        {
            ...
            if (this.o_currentResult == null)
            {
                throw new NullReferenceException("Current result is null - sqlite data reader cannot be read");
            }
            ...
            for (int i = 0; i < this.o_currentResult.FieldCount; i++)
            {
                ...
                if (this.o_currentResult.GetValue(i).GetType() == typeof(DBNull))
                {
                    s_dataTypeName = "null";
                }
                ...
                switch (s_dataTypeName.ToLower())
                {
                    case "varchar":
                    case "text":
                        if (ForestNET.Lib.Global.IsILevel((byte)ForestNET.Lib.Log.Level.MASS)) ForestNET.Lib.Global.ILogMass("get column value with SQLiteDataReader.GetString");
                        o_row.Add(s_name, this.o_currentResult.GetString(i));
                        break;
                    case "byte":
                        if (ForestNET.Lib.Global.IsILevel((byte)ForestNET.Lib.Log.Level.MASS)) ForestNET.Lib.Global.ILogMass("get column value with SQLiteDataReader.GetByte");
                        o_row.Add(s_name, this.o_currentResult.GetByte(i));
                        break;
                    case "smallint":
                        if (ForestNET.Lib.Global.IsILevel((byte)ForestNET.Lib.Log.Level.MASS)) ForestNET.Lib.Global.ILogMass("get column value with SQLiteDataReader.GetInt16");
                        o_row.Add(s_name, this.o_currentResult.GetInt16(i));
                        break;
                    case "integer":
                        if (ForestNET.Lib.Global.IsILevel((byte)ForestNET.Lib.Log.Level.MASS)) ForestNET.Lib.Global.ILogMass("get column value with SQLiteDataReader.GetInt32");
                        o_row.Add(s_name, this.o_currentResult.GetInt32(i));
                        break;
                    case "bigint":
                        if (ForestNET.Lib.Global.IsILevel((byte)ForestNET.Lib.Log.Level.MASS)) ForestNET.Lib.Global.ILogMass("get column value with SQLiteDataReader.GetInt64");
                        o_row.Add(s_name, this.o_currentResult.GetInt64(i));
                        break;
                    case "datetime":
                        if (ForestNET.Lib.Global.IsILevel((byte)ForestNET.Lib.Log.Level.MASS)) ForestNET.Lib.Global.ILogMass("get column value with SQLiteDataReader.GetDateTime");
                        ...
                        break;
                    case "time":
                        if (ForestNET.Lib.Global.IsILevel((byte)ForestNET.Lib.Log.Level.MASS)) ForestNET.Lib.Global.ILogMass("get column value with SQLiteDataReader.GetTimeSpan");
                        TimeSpan o_timeSpanFoo = TimeSpan.MinValue;

                        if (!(this.o_currentResult.GetValue(i).ToString() ?? "").ToLower().Equals("null"))
                        {
                            /* System.Data.SQLite seems to retrieve time column values as DateTime */
                            if (this.o_currentResult.GetValue(i).GetType() == typeof(DateTime))
                            {
                                /* read value as DateTime */
                                o_timeSpanFoo = this.o_currentResult.GetDateTime(i).TimeOfDay;
                            }
                            else
                            {
                                /* read value as String */
                                string? s_foo = this.o_currentResult.GetValue(i).ToString();

                                if (s_foo != null)
                                {
                                    o_timeSpanFoo = ForestNET.Lib.Helper.FromTimeString(s_foo);
                                }
                            }
                        }
                        ...
                        break;
                    case "float":
                        if (ForestNET.Lib.Global.IsILevel((byte)ForestNET.Lib.Log.Level.MASS)) ForestNET.Lib.Global.ILogMass("get column value with SQLiteDataReader.GetFloat");
                        o_row.Add(s_name, this.o_currentResult.GetFloat(i));
                        break;
                    case "double":
                        if (ForestNET.Lib.Global.IsILevel((byte)ForestNET.Lib.Log.Level.MASS)) ForestNET.Lib.Global.ILogMass("get column value with SQLiteDataReader.GetDouble");
                        o_row.Add(s_name, this.o_currentResult.GetDouble(i));
                        break;
                    case "decimal":
                        if (ForestNET.Lib.Global.IsILevel((byte)ForestNET.Lib.Log.Level.MASS)) ForestNET.Lib.Global.ILogMass("get column value with SQLiteDataReader.GetDecimal");
                        o_row.Add(s_name, this.o_currentResult.GetDecimal(i));
                        break;
                    case "bit":
                    case "bool":
                    case "boolean":
                        if (ForestNET.Lib.Global.IsILevel((byte)ForestNET.Lib.Log.Level.MASS)) ForestNET.Lib.Global.ILogMass("get column value with SQLiteDataReader.GetBoolean");
