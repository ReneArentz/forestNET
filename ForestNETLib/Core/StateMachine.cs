namespace ForestNETLib.Core
{
    ...
    public class StateMachine
    {
        ...
        public void AddTransition(string p_s_fromState, string p_s_returnCode, string p_s_toState)
        {
            ...
            {
                throw new ArgumentException("Invalid state '" + p_s_fromState + "', valid states are [" + ForestNETLib.Core.Helper.JoinList(this.a_states, ',') + "]");
            }

            if (!this.a_returnCodes.Contains(p_s_returnCode))
            {
                throw new ArgumentException("Invalid return code '" + p_s_returnCode + "', valid states are [" + ForestNETLib.Core.Helper.JoinList(this.a_returnCodes, ',') + "]");
            }

            if (!this.a_states.Contains(p_s_toState))
            {
                throw new ArgumentException("Invalid state '" + p_s_toState + "', valid states are [" + ForestNETLib.Core.Helper.JoinList(this.a_states, ',') + "]");
            }
        ...
        public void AddStateMethod(StateMethodContainer o_stateMethodContainer)
        {
            ...
            if (!b_found)
            {
                throw new InvalidOperationException("Invalid state '" + o_stateMethodContainer.State + "', valid states are [" + ForestNETLib.Core.Helper.JoinList(this.a_states, ',') + "]");
            }
