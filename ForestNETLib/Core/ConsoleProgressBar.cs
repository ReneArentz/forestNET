namespace ForestNETLib.Core
{
    ...
    public class ConsoleProgressBar
    {
        ...
        public double Report
        {
            set
            {
                if (this.o_progressBarTimerHandler != null)
                {
                    ...
                    if (ForestNETLib.Core.Global.IsILevel((byte)ForestNETLib.Log.Level.MASS)) ForestNETLib.Core.Global.ILogMass("Reported progress: '" + Math.Max(0, Math.Min(1, value)) + "'");
                }
            }
        }
        ...
        public ConsoleProgressBar(long p_l_animationInterval, int p_i_blockLength, int p_i_marqueeLength, int p_i_marqueeInterval)
        {
            ...
            ForestNETLib.Core.Global.ILogConfig("animationInterval: '" + this.l_animationInterval + "'");
            ForestNETLib.Core.Global.ILogConfig("blockLength: '" + this.i_blockLength + "'");
            ForestNETLib.Core.Global.ILogConfig("marqueeLength: '" + this.i_marqueeLength + "'");
            ForestNETLib.Core.Global.ILogConfig("marqueeInterval: '" + this.i_marqueeInterval + "'");
        }
        ...
        public void Init(string? p_s_startText, string? p_s_doneText, string? p_s_marqueeText)
        {
            ...
            ForestNETLib.Core.Global.ILogConfig("startText: '" + p_s_startText + "'");
            ForestNETLib.Core.Global.ILogConfig("doneText: '" + p_s_doneText + "'");
            ForestNETLib.Core.Global.ILogConfig("marqueeText: '" + p_s_marqueeText + "'");
            ...
            ForestNETLib.Core.Global.ILogFinest("ConsoleProgressBar timer started");
            ForestNETLib.Core.Global.ILogFinest("ConsoleProgressBar initialised");
        }
        ...
        public void Close()
        {
            ...
            if (this.o_timer != null)
            {
                ...
                ForestNETLib.Core.Global.ILogFinest("ConsoleProgressBar timer stopped");
            }

            ForestNETLib.Core.Global.ILogFinest("ConsoleProgressBar closed");
        }
        ...
        private class ProgressBarTimerHandler : Runnable
        {
            ...
            override public void Run()
            {
                ...
                try
                {
                    if (!ForestNETLib.Core.Helper.IsStringEmpty(this.s_startText))
                    {
                        s_foo = this.s_startText + " ";
                    }

                    /* implement marquee text if it has a value */
                    if ((this.s_marqueeText != null) && (!ForestNETLib.Core.Helper.IsStringEmpty(this.s_marqueeText)))
                    {
                ...
                catch (Exception o_exc)
                {
                    ...
                    ForestNETLib.Core.Global.LogException("Exception in rendering console progress bar with Update(String): ", o_exc);
                }
            }
            ...
            public void Close()
            {
                ...
                System.Console.Write(this.s_doneText + ForestNETLib.IO.File.NEWLINE);
            }
