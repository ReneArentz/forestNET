    ...
    public class ZIP
    {
        ...
        public static long GetSize(string p_s_sourceLocation)
        {
            ...
            using (ZipArchive o_zipInputStream = new(System.IO.File.OpenRead(p_s_sourceLocation), ZipArchiveMode.Read))
            {
                ...
                {
                    ForestNETLib.Core.Global.ILogMass("file length of '" + o_zipEntry.Name + "' = " + ForestNETLib.Core.Helper.FormatBytes(o_zipEntry.Length));
        ...
        public static bool CheckArchive(string p_s_sourceLocation, ZIP.PostProgress? p_del_postProgress)
        {
            ...
            /* check if source location really exists */
            if (!ForestNETLib.IO.File.Exists(p_s_sourceLocation))
            {
                throw new ArgumentException("Source location[" + p_s_sourceLocation + "] does not exist");
            }
            ...
            try
            {
                ...
                foreach (ZipArchiveEntry o_zipEntry in o_zipInputStream.Entries)
                {
                    ...
                    ForestNETLib.Core.Global.ILogMass("start checking file in archive '" + o_zipEntry.Name + "'");
                    ...
                    ForestNETLib.Core.Global.ILogMass("finished checking file in archive '" + o_zipEntry.Name + "'");
                }
            }
            catch (Exception)
            {
        ...
        public static void Zip(string p_s_sourceLocation, string p_s_destinationLocation, CompressionLevel p_o_compressionLevel = CompressionLevel.Optimal, ZIP.PostProgress? p_del_postProgress = null)
        {
            ...
            if (p_del_postProgress != null)
            {
                foreach (ListingElement o_listingElement in ForestNETLib.IO.File.ListDirectory(p_s_sourceLocation, true))
                {
            ...
            foreach (ListingElement o_listingElement in ForestNETLib.IO.File.ListDirectory(p_s_sourceLocation, true))
            {
                ...
                ZipArchiveEntry o_zipEntry = o_zipOutputStream.CreateEntry(o_listingElement.FullName.Substring(p_s_sourceLocation.LastIndexOf(ForestNETLib.IO.File.DIR) + 1), p_o_compressionLevel);

                ForestNETLib.Core.Global.ILogMass("start zipping file to archive '" + o_zipEntry.Name + "'");
                ...
                ForestNETLib.Core.Global.ILogMass("finished zipping file to archive '" + o_zipEntry.Name + "'");
            }
        }
        ...
        public static void Unzip(string p_s_sourceLocation, string p_s_destinationLocation, bool p_b_createDestinationPath, bool p_b_deleteSourceLocation, ZIP.PostProgress? p_del_postProgress)
        {
            ...
            if (!ForestNETLib.IO.File.FolderExists(p_s_destinationLocation))
            {
                ...
                else
                {
                    ...
                    ForestNETLib.IO.File.CreateDirectory(p_s_destinationLocation);
                }
            }

            /* check if destination location is a directory */
            if (!ForestNETLib.IO.File.IsDirectory(p_s_destinationLocation))
            {
            ...
            using (ZipArchive o_zipInputStream = new(System.IO.File.OpenRead(p_s_sourceLocation), ZipArchiveMode.Read))
            {
                ...
                foreach (ZipArchiveEntry o_zipEntry in o_zipInputStream.Entries)
                {
                    ...
                    if ((s_pathParentFolder != null) && (!ForestNETLib.IO.File.FolderExists(s_pathParentFolder)))
                    {
                        /* create path */
                        ForestNETLib.IO.File.CreateDirectory(s_pathParentFolder);
                    }
                    ...
                    try
                    {
                        ...
                        using (o_fileStream = System.IO.File.OpenWrite(System.IO.Path.Combine(p_s_destinationLocation, o_zipEntry.FullName)))
                        {
                            ...
                            ForestNETLib.Core.Global.ILogMass("start unzipping file from archive '" + o_zipEntry.Name + "'");
                            ...
                            ForestNETLib.Core.Global.ILogMass("finished unzipping file from archive '" + o_zipEntry.Name + "'");
                        }
                    }
                    catch (Exception o_exc)
                    {
                        throw new System.IO.IOException("Could not create file[" + p_s_destinationLocation + ForestNETLib.IO.File.DIR + o_zipEntry.Name + "]; " + o_exc.Message);
                    }
                }
            }
            ...
            if (p_b_deleteSourceLocation)
            {
                ...
                ForestNETLib.IO.File.DeleteFile(p_s_sourceLocation);
            }
        }
    }
}
