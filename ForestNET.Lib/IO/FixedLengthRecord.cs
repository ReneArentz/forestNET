namespace ForestNET.Lib.IO
{
    ...
    public abstract class FixedLengthRecord : IFixedLengthRecord
    {
        ...
        public FixedLengthRecord()
        {
            ...
            if (ForestNET.Lib.Global.IsILevel((byte)ForestNET.Lib.Log.Level.MASS)) ForestNET.Lib.Global.ILogMass("created fixed length record object");
        }
        ...
        public IFixedLengthRecord ReadFieldsFromString(string p_s_line)
        {
            ...
            foreach (KeyValuePair<int, StructureElement> o_structureElement in this.Structure)
            {
                ...
                if (!ForestNET.Lib.Helper.IsStringEmpty(o_structureElement.Value.Constant))
                {
                ...
                else /* we have a normal structure element */
                {
                    ...
                    if ((!ForestNET.Lib.Helper.IsStringEmpty(s_value.Trim())) && (!ForestNET.Lib.Helper.MatchesRegex(s_value, "^[0]+$")))
                    {
                        if (ForestNET.Lib.Global.IsILevel((byte)ForestNET.Lib.Log.Level.MASS)) ForestNET.Lib.Global.ILogMass("field '" + o_structureElement.Value.Field + "(" + i_position + ".." + (i_position + o_structureElement.Value.Length) + ")'\t\tunique '" + (this.Unique.Contains(o_structureElement.Value.Field ?? "") ? "yes" : "no ") + "'\t\tvalue '" + s_value + "'\t\tconverted value '" + ((o_structureElement.Value.ReadTranspose == null) ? o_structureElement.Value.ReadFPNTranspose?.Invoke(s_value, o_structureElement.Value.PositionDecimalSeparator) : o_structureElement.Value.ReadTranspose.Invoke(s_value)) + "'");
                    ...
                    else if ((!ForestNET.Lib.Helper.IsStringEmpty(s_value.Trim())) && (ForestNET.Lib.Helper.MatchesRegex(s_value, "^[0]+$")))
                    { /* value has only character '0' */
                        ...
                        {
                            if (ForestNET.Lib.Global.IsILevel((byte)ForestNET.Lib.Log.Level.MASS)) ForestNET.Lib.Global.ILogMass("field '" + o_structureElement.Value.Field + "(" + i_position + ".." + (i_position + o_structureElement.Value.Length) + ")'\t\tunique '" + (this.Unique.Contains(o_structureElement.Value.Field ?? "") ? "yes" : "no ") + "'\t\tvalue '" + s_value + "'\t\tconverted value 'null'");
        ...
        public string WriteFieldsToString()
        {
            ...
            foreach (KeyValuePair<int, StructureElement> o_structureElement in this.Structure)
            {
                ...
                else if (!ForestNET.Lib.Helper.IsStringEmpty(o_structureElement.Value.Constant)) /* check if we have a constant field */
                {
