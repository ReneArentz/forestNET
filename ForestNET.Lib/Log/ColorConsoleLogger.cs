...
namespace ForestNET.Lib.Log
{
    ...
    public class ColorConsoleLoggerConfiguration
    {
        ...
        {
            if (ForestNET.Lib.Helper.IsStringEmpty(p_s_consoleColor))
            {
        ...
        public void Log<TState>(LogLevel p_o_logLevel, EventId p_o_eventId, TState p_o_state, Exception? p_o_exception, Func<TState, Exception, string> p_o_formatter)
        {
            ...
            if (this.o_config.EventId == 0 || this.o_config.EventId == p_o_eventId.Id)
            {
                ...
                if (ForestNET.Lib.Helper.IsInteger(s_log.Substring(0, 1)))
                {
                    ...
                    ConsoleColor e_originalColor = System.Console.ForegroundColor;

                    /* use configured console color based on log level map */
                    System.Console.ForegroundColor = this.o_config.LogLevelToColorMap[o_logLevel];
                    /* add datetime stamp to console */
                    System.Console.Write($"[" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff", System.Globalization.CultureInfo.InvariantCulture) + "] ");
                    /* add log level to console */
                    System.Console.Write($"[{o_logLevel,-11}] ");
                    ...
                    if (!ForestNET.Lib.Helper.IsStringEmpty(s_message))
                    {
                        ...
                        if (s_message.StartsWith("fNET§§§"))
                        {
                            ...
                            if (!ForestNET.Lib.Helper.IsStringEmpty(a_messageParts[3]))
                            {
                                ...
                                System.Console.Write("[" + s_sourcePath + "] ");
                            }
                            else
                            {
                                string s_sourcePath = "";
                            ...
                            System.Console.Write(a_messageParts[1].Replace(Environment.NewLine, " "));
                            /* add exception to console, removing any newline characters */
                            System.Console.Write(a_messageParts[2].Replace(Environment.NewLine, " "));
                        }
                        else /* just a usual log message */
                        {
                            /* add log message to console, removing any newline characters */
                            System.Console.Write(s_message.Replace(Environment.NewLine, " "));
                        }
                    }
                    ...
                    System.Console.ForegroundColor = e_originalColor;
                }
            }
        }
    }
