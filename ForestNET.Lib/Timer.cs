    ...
    public class Timer
    {
        ...
        public async void StartTimer()
        {
            if (this.o_timerTask.StartTime != System.TimeSpan.Zero)
            {
                ...
                ForestNET.Lib.Global.ILogConfig("time now: '" + o_localTime + "'");
                ForestNET.Lib.Global.ILogConfig("start delay calculation with start time: '" + this.o_timerTask.StartTime + "'");
                ...
                ForestNET.Lib.Global.ILogConfig("calculated start delay: '" + i_startDelay + " ms'");

                /* if calculated start delay is negative, it will be set for next day */
                if (i_startDelay < 0)
                {
                    i_startDelay += 24 * 60 * 60 * 1_000;
                    ForestNET.Lib.Global.ILogConfig("start delay is negative, it will be set for next day: '" + System.DateTime.Now.AddSeconds(i_startDelay / 1_000) + "'");
                }
            ...
            this.o_timer.Start();
            ForestNET.Lib.Global.ILogFinest("timer started");
        }
        ...
        public void StopTimer()
        {
            ...
            if (this.o_timer != null)
            {
                ...
                ForestNET.Lib.Global.ILogFinest("timer stopped");
            }
        }
    }
}
