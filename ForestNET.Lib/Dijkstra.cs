namespace ForestNET.Lib
{
    ...
    public class Dijkstra<T> where T : notnull
    {
        ...
        public void Add(T p_o_node, T p_o_to, int p_i_weight)
        {
            ...
            ForestNET.Lib.Global.ILogConfig("added node with index '" + this.a_mapping.IndexOf(p_o_node) + "', to node '" + this.a_mapping.IndexOf(p_o_to) + "' and weight '" + p_i_weight + "'");
        }
        ...
        public void ExecuteDijkstra(T p_o_startNode)
        {
            ...
            ForestNET.Lib.Global.ILogMass("set all distances to 0");
            ...
            for (int i = 0; i < i_size; i++)
            {
                ...
                ForestNET.Lib.Global.ILogMass("set node '" + i + "' to standard values: max weights, predecessors -1, visited false and first distances");
            }
            ...
            if (this.a_mapping.Contains(p_o_startNode))
            {
                ...
                ForestNET.Lib.Global.ILogMass("start node '" + p_o_startNode + "' shortest path is '0'");
            }
            ...
            for (int i = 0; i < i_amount; i++)
            {
                ...
                for (int j = 0; j < i_amount; j++)
                {
                    ...
                    {
                        ForestNET.Lib.Global.ILogMass("set min distance '" + j + "', because '" + j + "' was not visited and min distance is '-1', or weight sum to '" + j + "(" + a_shortestPaths[j] + ")' is lower than " + ((i_minDistance >= 0) ? a_shortestPaths[i_minDistance].ToString() : "infinity"));
                        i_minDistance = j;
                    }
                }

                a_visited[i_minDistance] = true;
                ForestNET.Lib.Global.ILogMass(i_minDistance + " was visited just now");

                for (int j = 0; j < i_amount; j++)
                {
                    ForestNET.Lib.Global.ILogMass("check distance from '" + i + "' to '" + j + "'");
                    if (a_distances[i_minDistance, j] != 0)
                    {
                        if (a_shortestPaths[i_minDistance] + a_distances[i_minDistance, j] < a_shortestPaths[j])
                        {
                            ForestNET.Lib.Global.ILogMass("distance from '" + i_minDistance + "' to '" + j + "': " + a_distances[i_minDistance, j] + " != 0");
                            ForestNET.Lib.Global.ILogMass("weight sum(" + a_shortestPaths[i_minDistance] + ") from '" + i_minDistance + "' plus distance from '" + i_minDistance + "' to '" + j + "' [" + a_distances[i_minDistance, j] + "] is lower than weight sum(" + a_shortestPaths[j] + ") from '" + j + "'");
                            ForestNET.Lib.Global.ILogMass("update weight sum(" + a_shortestPaths[j] + ") from '" + j + "' to (" + (int)(a_shortestPaths[i_minDistance] + a_distances[i_minDistance, j]) + ")");
                            ForestNET.Lib.Global.ILogMass("update predecessors(" + a_predecessors[j] + ") from '" + j + "' to (" + (int)(i_minDistance + 1) + ")");
            ...
            for (int i = 0; i < i_amount; i++)
            {
                ...
                ForestNET.Lib.Global.ILogFinest("added weight sum of shortest path '" + a_shortestPaths[i] + "' from start node '" + this.a_mapping.IndexOf(p_o_startNode) + " to destination node '" + i + "'");
            }
            ...
            for (int i = 0; i < i_amount; i++)
            {
                ...
                ForestNET.Lib.Global.ILogFinest("added destination node '" + this.a_mapping[i_to] + "(" + i_to + ")' to shortest path");
                ...
                while (a_predecessors[i_to] != -1)
                {
                    ...
                    ForestNET.Lib.Global.ILogFinest("added predecessors node '" + this.a_mapping[i_to] + "(" + i_to + ")' to shortest path");
                }

                /* reverse shortest path list */
                a_path.Reverse();
                ForestNET.Lib.Global.ILogMass("reverse shortest path for right order from start node '" + this.a_mapping.IndexOf(p_o_startNode) + "' to destination node '" + i + "'");

                /* add shortest path list to property list with destination node as key */
                this.a_shortestPaths[this.a_mapping[i]] = a_path;
                ForestNET.Lib.Global.ILogFinest("added shortest path " + a_path + " to class property");
            }
        }
