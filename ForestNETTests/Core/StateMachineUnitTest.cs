    ...
    public class StateMachineUnitTest
    {
        ...
        public void TestStateMachine()
        {
            try
            {
                ...
                string s_testDirectory = s_currentDirectory + ForestNETLib.IO.File.DIR + "testStateMachine" + ForestNETLib.IO.File.DIR;
                ...
                ForestNETLib.Core.StateMachine o_stateMachine = new(a_states, a_returnCodes);
                ...
                o_stateMachine.AddTransition("FINALSTATE", ForestNETLib.Core.StateMachine.EXIT, ForestNETLib.Core.StateMachine.EXIT);

                /* define state machine methods */
                o_stateMachine.AddStateMethod(new ForestNETLib.Core.StateMachine.StateMethodContainer(
                    "BEGINSTATE",
                    p_a_genericList =>
                    {
                        p_a_genericList[1] = ((int)p_a_genericList[1]) + 1;

                        int i_random = ForestNETLib.Core.Helper.RandomIntegerRange(1, 100);

                        if ((i_random >= 26) && (i_random <= 50))
                        {
                            ((ForestNETLib.IO.File)p_a_genericList[0]).AppendLine("BEGINSTATE");
                            return "START";
                        }
                        else
                        {
                            ((ForestNETLib.IO.File)p_a_genericList[0]).AppendLine("BEGINSTATE - IDLE");
                ...
                o_stateMachine.AddStateMethod(new ForestNETLib.Core.StateMachine.StateMethodContainer(
                    ...
                    p_a_genericList =>
                    {
                        ((ForestNETLib.IO.File)p_a_genericList[0]).AppendLine("STATE1");
                        p_a_genericList[1] = ((int)p_a_genericList[1]) + 1;
                        return "TRANSFER";
                    }
                ));
                ...
                o_stateMachine.AddStateMethod(new ForestNETLib.Core.StateMachine.StateMethodContainer(
                    ...
                    p_a_genericList =>
                    {
                        ((ForestNETLib.IO.File)p_a_genericList[0]).AppendLine("STATE2");
                        p_a_genericList[1] = ((int)p_a_genericList[1]) + 1;
                        return "TRANSFER";
                    }
                ));
                ...
                o_stateMachine.AddStateMethod(new ForestNETLib.Core.StateMachine.StateMethodContainer(
                    ...
                    p_a_genericList =>
                    {
                        ((ForestNETLib.IO.File)p_a_genericList[0]).AppendLine("STATE3");
                        p_a_genericList[1] = ((int)p_a_genericList[1]) + 1;
                        return "TRANSFER";
                    }
                ));
                ...
                o_stateMachine.AddStateMethod(new ForestNETLib.Core.StateMachine.StateMethodContainer(
                    ...
                    p_a_genericList =>
                    {
                        ((ForestNETLib.IO.File)p_a_genericList[0]).AppendLine("FINALSTATE");
                        p_a_genericList[1] = ((int)p_a_genericList[1]) + 1;
                        return ForestNETLib.Core.StateMachine.EXIT;
                    }
                ));

                /* execute state machine until EXIT state */
                string s_returnCode = ForestNETLib.Core.StateMachine.EXIT;
                ...
                } while (!s_returnCode.Equals(ForestNETLib.Core.StateMachine.EXIT));

                int i_amountLines = (int)a_genericList[1];
                int i_amountIDLE = (int)a_genericList[2];
                ForestNETLib.IO.File o_checkFile = new(s_file);
        ...
        public void TestStateMachineWithTimer()
        {
            ...
            {
                string s_currentDirectory = System.IO.Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly().Location) ?? throw new NullReferenceException("Current directory could not be resolved with '" + System.Reflection.Assembly.GetExecutingAssembly().Location + "'");
                string s_testDirectory = s_currentDirectory + ForestNETLib.IO.File.DIR + "testStateMachineWithTimer" + ForestNETLib.IO.File.DIR;
                ...
                ForestNETLib.Core.Timer o_timer = new(o_task);
        ...
        private class StateMachineTimerTask : ForestNETLib.Core.TimerTask
        {

            /* Fields */

            private ForestNETLib.Core.StateMachine? o_stateMachine;
            ...
            public StateMachineTimerTask(ForestNETLib.Core.DateInterval p_o_interval, string p_s_logFile) : base(p_o_interval)
            {
                this.BuildStateMachine(p_s_logFile);
            }

            public StateMachineTimerTask(ForestNETLib.Core.DateInterval p_o_interval, System.TimeSpan p_o_startTime, string p_s_logFile) : base(p_o_interval, p_o_startTime)
            {
                this.BuildStateMachine(p_s_logFile);
            }
            ...
            private void BuildStateMachine(string p_s_logFile)
            {
                try
                {
                    ForestNETLib.IO.File o_file = new(p_s_logFile);
                    ...
                    this.o_stateMachine.AddTransition("RED", ForestNETLib.Core.StateMachine.EXIT, ForestNETLib.Core.StateMachine.EXIT);
                    ...
                    /* define state machine methods */
                    this.o_stateMachine.AddStateMethod(new ForestNETLib.Core.StateMachine.StateMethodContainer(
                        "RED",
                        p_a_genericList =>
                        {
                            p_a_genericList[1] = ((int)p_a_genericList[1]) + 1;
                            ...
                            {
                                ((ForestNETLib.IO.File)p_a_genericList[0]).AppendLine("RED - CIRCLE_END");
                                p_a_genericList[2] = ((int)p_a_genericList[2]) + 1;
                                return "CIRCLE_END";
                            }
                            else if ((i_random >= 41) && (i_random <= 75))
                            {
                                ((ForestNETLib.IO.File)p_a_genericList[0]).AppendLine("RED - CAR_CONTACT");
                                return "CAR_CONTACT";
                            }
                            else
                            {
                                ((ForestNETLib.IO.File)p_a_genericList[0]).AppendLine("RED - EXIT");
                                return "EXIT";
                            }
                        }
                    ));
                    ...
                    this.o_stateMachine.AddStateMethod(new ForestNETLib.Core.StateMachine.StateMethodContainer(
                        ...
                        p_a_genericList =>
                        {
                            ((ForestNETLib.IO.File)p_a_genericList[0]).AppendLine("RED_YELLOW");
                            p_a_genericList[1] = ((int)p_a_genericList[1]) + 1;
                            return "CIRCLE_END";
                        }
                    ));
                    ...
                    this.o_stateMachine.AddStateMethod(new ForestNETLib.Core.StateMachine.StateMethodContainer(
                        ...
                        p_a_genericList =>
                        {
                            ((ForestNETLib.IO.File)p_a_genericList[0]).AppendLine("GREEN");
                            p_a_genericList[1] = ((int)p_a_genericList[1]) + 1;
                            return "CIRCLE_END";
                        }
                    ));
                    ...
                    this.o_stateMachine.AddStateMethod(new ForestNETLib.Core.StateMachine.StateMethodContainer(
                        ...
                        p_a_genericList =>
                        {
                            ((ForestNETLib.IO.File)p_a_genericList[0]).AppendLine("YELLOW");
                            p_a_genericList[1] = ((int)p_a_genericList[1]) + 1;
                            return "CIRCLE_END";
                        }
                    ));
                    ...
                    /* define init values */
                    this.s_returnCode = ForestNETLib.Core.StateMachine.EXIT;
            ...
            override public void RunTimerTask()
            {
                ...
                if ((this.s_returnCode == null) || (this.s_returnCode.Equals(ForestNETLib.Core.StateMachine.EXIT)))
                {
                    this.Stop = true;
                }
